...
describe('DataTable — bulk actions (FC-1.4)', () => {
  it('renders no checkboxes or action bar unless the bulkActions prop is provided', () => {
    render(<DataTable columns={columns} rows={rows} keyOf={(r) => r.id} />);
  ...
  it('renders a checkbox per row plus a header "select all" checkbox', () => {
    render(<DataTable columns={columns} rows={rows} keyOf={(r) => r.id} bulkActions={{ renderActions: () => <button>Delete</button> }} />);
  ...
  it('passes the selected row objects (not just ids) to renderActions', async () => {
    let received: Row[] = [];
    const user = userEvent.setup();
    render(
      <DataTable
  ...
  it('clears selection when clearSelection is called', async () => {
    ...
    render(
      <DataTable
        ...
        keyOf={(r) => r.id}
        bulkActions={{
          renderActions: (_selectedRows, clearSelection) => <button onClick={clearSelection}>Clear</button>,
        }}
      />,
    );
  ...
  it('the header checkbox selects and deselects every row on the current page', async () => {
    const user = userEvent.setup();
    render(<DataTable columns={columns} rows={rows} keyOf={(r) => r.id} bulkActions={{ renderActions: () => <button>Delete</button> }} />);
  ...
  it('resets selection when the rows array reference changes', async () => {
    const user = userEvent.setup();
    const { rerender } = render(
      <DataTable columns={columns} rows={rows} keyOf={(r) => r.id} bulkActions={{ renderActions: () => <button>Delete</button> }} />,
    );
    ...
    const newRows = [...rows];
    rerender(<DataTable columns={columns} rows={newRows} keyOf={(r) => r.id} bulkActions={{ renderActions: () => <button>Delete</button> }} />);

    expect(screen.queryByText(/selected/)).not.toBeInTheDocument();
  });

  it('preserves selection across a page change', async () => {
    const manyRows: Row[] = Array.from({ length: 25 }, (_, i) => ({ id: String(i + 1), name: `Row ${i + 1}`, amount: i }));
    const user = userEvent.setup();
    render(<DataTable columns={columns} rows={manyRows} keyOf={(r) => r.id} bulkActions={{ renderActions: () => <button>Delete</button> }} />);

    await user.click(screen.getByLabelText('Select row 1'));
    await user.click(screen.getByRole('button', { name: 'Next' }));

    // Selection made on page 1 should still count, even while viewing page 2.
    expect(screen.getByText('1 selected')).toBeInTheDocument();
  });
});
