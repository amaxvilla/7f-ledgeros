...
interface FixedAssetRegisterResponse {
  entityId: string;
  assetCount: number;
  byCategory: Record<
    string,
...
/** Reused verbatim from `/fixed-assets/page.tsx`'s own map â€” same three
...
/** Reused verbatim from `/payments/page.tsx`'s own map â€” same four
...
/** `MonoLinkStatus` â€” confirmed directly against `schema.prisma` (NOT
 ...
 *  for this one to import â€” mirrored, not reused, same "no shared
 ...
 *  â€” confirmed directly in `vw_payment_transactions_register.sql`'s own
 ...
 *  is already major-unit â€” this conversion stays local to the one
...
function agingColumns<T extends AgingRow>(counterpartyLabel: string, counterpartyName: (r: T) => string) {
  return [
    ...
    { header: 'Due date', render: (r: T) => (r.due_date ? new Date(r.due_date).toLocaleDateString() : 'â€”') },
...
/**
 * Frontend Completion, FE-9.4 â€” a new `/reports` hub, FE-9.3's own
 ...
 * Aging picked as the first pair â€” both single-`entityId`-param `GET`s
 * (confirmed directly against `ReportingController.vendorAging`/
 * `.customerAging`), no fiscal-period selector needed at all, unlike
 * every statement on `/financial-statements` â€” genuinely the smallest
 ...
 * the service alone, since `$queryRaw`'s own return type is untyped) â€”
 ...
 * `AccountsReceivableService.getAging()` â€” this page trusts the view's
 ...
 * than two near-duplicate column/total sets â€” `VendorAgingRow`/
 ...
 * `ReportingController.vendorAging`/`.customerAging`) â€” the SAME two
 ...
 * (`agingTotals`) rather than a separate summary endpoint â€” no such
...
/**
 * Frontend Completion, FE-9.5 â€” the next well-matched pair FE-9.4's own
 ...
 * `GET`s, same shape as the aging pair â€” no fiscal-period selector
 * needed here either.
 *
 ...
 * â€” `margin_pct` is `NULL` when `revenue_amount` is zero (division
 * avoided at the SQL level, not left to produce `Infinity`/`NaN`
 * client-side), rendered as "â€”" rather than `0%` or `NaN%` for that
 * case. `budget.view` (confirmed directly) â€” the same permission
 * `/budgeting` already requires, not a new code.
 *
 * `vw_bank_reconciliation_summary` (read directly): one row per ACTIVE
 * bank account (`WHERE ba."isActive" = TRUE`, confirmed directly â€” an
 ...
 * any â€” `session_id`/`session_status`/`session_date` are all nullable,
 ...
 * (`DRAFT`/`APPROVED` â€” confirmed directly against the
 ...
 * to at all â€” a real, checked distinction, not assumed from the enum's
 * generic-sounding name). `bankrecon.view` (confirmed directly) â€” a
 ...
 * inside) `loadAgingReports` above â€” that function is left completely
...
/**
 * Frontend Completion, FE-9.6 â€” FE-9.5's own recommendation, read
 ...
 * â€” the third candidate FE-9.5 itself named â€” was READ directly too and
 * DELIBERATELY DROPPED from this checkpoint: `ReportingService.pmoRiskIssueRegister`
 * (confirmed directly) is not a list-shaped register at all â€” it's a
 ...
 * `/project-risks`/`/project-issues` already show â€” noted in this
 ...
 * rows per entity, one per fixed horizon (30/60/90 days) â€” confirmed
 ...
 * a KPI-card summary ACROSS the three rows would double-count â€” this is
 ...
 * yet, confirmed directly â€” a new asset with no depreciation history is
...
async function loadCashAndAssetReports(entityId: string) {
  const [cashForecast, fixedAssetRegister] = await Promise.all([
    ...
    fetchApi<FixedAssetRegisterResponse>(`/reporting/fixed-asset-register?entityId=${entityId}`),
  ]);
  return { cashForecast, fixedAssetRegister };
}

/**
 * Frontend Completion, FE-9.7 â€” Payment Transactions Register + Mono
 ...
 *
 * **It does â€” more so.** `ReportingService.pmoProjectPerformance` (read
 * directly) is `{ entityId, projectId, schedule, earnedValue }`, exactly
 * `SchedulingService.getGanttData` + `.computeEarnedValue` combined â€”
 ...
 * "worth checking" maybe â€” a CONFIRMED duplicate of an already-shipped
 * page. Dropped from this checkpoint entirely, same as
 * `pmoRiskIssueRegister` before it â€” remove both from any future
 ...
 * (`vw_mono_linked_accounts_register`, read directly) â€” both confirmed
 * genuine `$queryRaw` list views, single-`entityId`-param plus an
 * optional `status` filter (NOT applied client-side this checkpoint â€”
 ...
 *   (`PaymentsService.listTransactions`, a plain `findMany` â€” confirmed
 ...
 *   performs â€” the same "Dashboard/Reporting split" both views'
 ...
 *   overlaps more â€” `mono-linked-accounts-register`'s own extra columns
 ...
 *   one-off judgment call) â€” but named here plainly as the weaker,
 ...
 * cents) â€” confirmed directly in the view's own SQL comment, and by
 ...
 * link status would be `MonoLinkedAccountStatus` â€” that name does not
 ...
 * overview) â€” both confirmed already-existing permission codes, no new
 * one introduced.
 */
...
/**
 * Agent & Commission Management, RE-COMM.5 (frontend) â€” the five
 ...
 * that controller) â€” the same read permission `/commission-calculations`
 ...
 *
 * `by-project`'s own response nests a `units` array per project â€” this
  ...
  return (
    <PageContainer>
        ...
        (() => {
          ...
          return (
            <section style={{ marginBottom: tokens.space(8) }}>
              ...
              <DataTable
                columns={[
                  { header: 'Project', render: (r: ProjectProfitabilityRow) => `${r.project_code} â€” ${r.project_name}` },
                  ...
                  },
                  {
                    header: 'Margin',
                    align: 'right' as const,
                    render: (r: ProjectProfitabilityRow) => (r.margin_pct === null ? 'â€”' : `${(r.margin_pct * 100).toFixed(1)}%`),
                  },
                ]}
                rows={rows}
                ...
                columns={[
                  ...
                  },
                  {
                    ...
                    render: (r: BankReconciliationSummaryRow) =>
                      r.session_status ? <Badge tone={r.session_status === 'APPROVED' ? 'positive' : 'neutral'}>{r.session_status}</Badge> : 'â€”',
                  },
                  {
      ...
      {evenMoreData &&
        (() => (
          <section style={{ marginBottom: tokens.space(8) }}>
            <PageHeader title="Cash Forecast" subtitle="Cumulative AP/AR due within each horizon â€” not discrete windows" />
            <DataTable
              columns={[
      ...
      {evenMoreData &&
        (() => {
          const rows = evenMoreData.fixedAssetRegister.assets;
              ...
              <DataTable
                columns={[
                  ...
                  { header: 'Acquisition cost', align: 'right' as const, render: (r: FixedAssetRegisterRow) => formatCurrency(r.acquisition_cost) },
                  {
                    ...
                    render: (r: FixedAssetRegisterRow) => (r.last_depreciated_period ? new Date(r.last_depreciated_period).toLocaleDateString() : 'â€”'),
                  },
            ...
            <section style={{ marginBottom: tokens.space(8) }}>
              ...
              <DataTable
                columns={[
                  ...
                  { header: 'Paid at', render: (r: PaymentTransactionRegisterRow) => (r.paid_at ? new Date(r.paid_at).toLocaleDateString() : 'â€”') },
                ]}
                rows={rows}
        ...
        (() => {
          ...
          return (
            <section>
              ...
              <DataTable
                columns={[
                  { header: 'Institution', render: (r: MonoLinkedAccountRegisterRow) => r.institution_name ?? 'â€”' },
                  { header: 'Account', render: (r: MonoLinkedAccountRegisterRow) => r.account_number_masked ?? 'â€”' },
                  { header: 'Currency', render: (r: MonoLinkedAccountRegisterRow) => r.currency ?? 'â€”' },
      ...
      {commissionData && (
        <section style={{ marginBottom: tokens.space(8) }}>
          ...
          <DataTable
            columns={[
              { header: 'Agent', render: (r: CommissionByAgentRow) => `${r.agentCode} â€” ${r.agentName}` },
        ...
        (() => {
          ...
          return (
            <section style={{ marginBottom: tokens.space(8) }}>
              ...
              <DataTable
                columns={[
                  { header: 'Agent', render: (r: CommissionAgingRow) => `${r.agentCode} â€” ${r.agentName}` },
      ...
      {commissionData && (
        <section>
          <PageHeader title="Commission Forecast" subtitle="Unpaid pipeline by lifecycle stage â€” nearest to payment first" />
          <DataTable
            columns={[
